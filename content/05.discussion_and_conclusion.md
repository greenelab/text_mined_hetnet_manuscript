...
When training relationship extractors for biomedical knowledge graphs, one should be cautious about the language used to describe relationship types.
For example, if a relationship is broad it is important to consider all the possible textual features that may be used. 
As the knowledge graph increases the amount of work required scales linearly.
When using this framework one should expect to have at least 25-30 different label functions per relationship type.
Along with label function construction performance does depend on the corpus and named entity recognition applied, so be aware of these cofounders.

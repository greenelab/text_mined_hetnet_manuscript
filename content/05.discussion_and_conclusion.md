...
<div style="color:red">
Make sure this section talks about how this framework can generalize.
Make sure the message includes the biomedical challenges and 
It is not discussed what challenges exist in biomedical how the proposed method addresses the challenges, why such a method can perform well in biomedical text.
</div>
...
<div style="color:red">
Approach off-diagonal prediction with caution. Scale is likely to grow linearly with the number of edges.
The number of label functions before we observed before performance plateaus varied and may be task-specific. Expect ~30/relationship type
The performance will depend in part on the quality of the underlying corpus and the named entity recognition applied to that
</div>

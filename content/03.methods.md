...
This model uses the following parameters to generate training estimates: a weight for the l2 loss, a learning rate, and the number of epochs.
We fixed the learning rate to be 1e-3 as any higher weight would explode the gradient.
We also fixed the number of epochs to 250 and performed a grid search for l2 loss.
This grid search used a grid of five evenly spaced numbers between 0.01 and 5.
Following the training phase, we used a threshold of 0.5 for discretizing our training classes' probability estimates within our analysis.
...
In this context of text mining, these networks are usually a transformer model [@arxiv:1706.03762].
These models have been shown to achieve high-performing results, which lead us to choose BioBERT [@arxiv:1901.08746] as our discriminative model.
...
We constructed confidence intervals using the 50 samples for each point within our sampling approach.

...
We hand labeled five hundred to a thousand candidate sentences of each relationship to obtain a ground truth set (Table {@tbl:candidate-sentences})[^1].
...
A common challenge in natural language processing is having a miniscule amount of ground truth annotations, even when textual data are abundant.
...
If the candidate pair is present, our label function will emit a positive label and abstain otherwise.
If the candidate pair isn't present in any existing database, then a separate label function will emit a negative label.
We use a separate label function to prevent a label imbalance problem. This problem occurs when candidates, that scarcely appear in databases, are drowned out by negative labels.
...
These filters generate a feature map that is sent into a maximum pooling layer that is designed to extract the largest feature in each map.
...
From the fully connected network the last step is to generate predictions using a softmax layer.
...
Zero being the same as distant supervision and one meaning all sentences are labeled.

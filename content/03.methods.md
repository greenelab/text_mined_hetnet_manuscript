...
For this project, we analyzed performance over a subset of the Hetionet v1 edge types: disease associates with a gene (DaG), compound binds to a gene (CbG), gene interacts with gene (GiG) and compound treating a disease (CtD) (bolded in Figure {@fig:hetionet}).
...
We describe each category and provide examples that refer to the following candidate sentence: "[PTK6]{.gene_color} may be a novel therapeutic target for [pancreatic cancer]{.disease_color}".
...
**Domain Heuristics**: These label functions use results from published text-based analyses to generate a signal. 
...
The discriminative model is a neural network trained to produce classification labels by integrating  predicted probabilities from the generative model as well as with sentence representations via word embeddings.
...
We evaluated both generative and discriminative models at each point, and report performance of each in terms of the area under the receiver operating characteristic curve (AUROC) and the area under the precision-recall curve (AUPR).
Following the model evaluations, we quantified the number of edges we could incorporate into Hetionet v1.
...
We report the number of preexisting edges we could recall as well as the number of novel edges we can add. 
Lastly, we compared our framework with an unsupervised approach [@tag:cocoscore].

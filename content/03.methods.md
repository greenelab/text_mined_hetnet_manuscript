...
Hetionet v1 [@doi:10.7554/eLife.26726] is a sizeable heterogeneous network that contains pharmacological and biological information.
This network depicts information in the form of nodes and edges of different types: nodes representing biological and pharmacological entities and edges representing relationships between entities. 
...
PubTator Central provides MEDLINE abstracts that have been annotated with well-established entity recognition tools including Tagger One [@doi:10.1093/bioinformatics/btw343] for disease, chemical and cell line mentions, tmVar [@doi:10.1093/bioinformatics/btx541] for variant tagging, GNormPlus [@doi:10.1155/2015/918710] for gene mentions and SR4GN [@doi:10.1371/journal.pone.0038460] for species mentions.
We downloaded PubTator Central on March 1, 2020, at which point it contained approximately 30,000,000 abstracts.
After downloading, we filtered out mention tags that were not contained in Hetionet v1.
We extracted sentences with two or more mentions and termed these sentences as candidate sentences.
...
Each candidate sentence was stratified by their corresponding abstract to produce a training set, tuning set, and a testing set.
This version of stratification prevents training issues such as data leakage, which could occur from sorting individual sentences.
Our stratification process uses random assortment to assign dataset labels to each abstract. 
...
<div style="color:red">
Update table and then open for review
</div>

| Relationship | Train | Tune | Test |
| :--- | :---: | :---: | :---: |
| Disease Associates Gene | 2.35 M | 331K (397+, 603-) | 313K (351+, 649-) |
...
These functions can use different approaches or techniques to emit a label; however, these functions can be grouped into simple categories.
Once constructed, these functions are combined using a generative model to output a single annotation. 
A consensus probability score is bounded between 0 (low chance of mentioning a relationship) and 1 (high chance of mentioning a relationship).
We used these annotations to train a discriminative model for the final classification step.

#### Label Function Categories

Label functions can be constructed in many ways; however, many label functions share similar characteristics.
We grouped these characteristics into the following categories: databases and text patterns.
Most of our label functions fall into the text pattern category, while the others fall into the database category (Supplemental Table {@tbl:label-functions}).
...
Our label function emits a positive label and abstains otherwise if the pair is present.
If the pair is not present in any existing database, a separate label function emits a negative label.
We used a separate label function to prevent a label imbalance problem we encountered during development: emitting positives and negatives from the same label function causes downstream classifiers to generate almost exclusively negative predictions.
...
Furthermore, a label function could utilize previously identified text patterns such as dependency parse trees[@tag:global_network]. 

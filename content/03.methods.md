...
The generative model produces predicted probabilities for each sentence by integrating labels.
The discriminative model is a neural network trained to produce the labels predicted from the generative model from sentence representations.
...
We built a network with multiple filters using a fixed width of 300 dimensions and a fixed height of 7 (Figure {@fig:convolutional_network}).

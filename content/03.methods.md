...
The discriminative model is a neural network, which we trained using labels predicted from the generative model.
The expectation is that the discriminative model can improve upon the generative model's predictions by incorporating textual features along with generated training labels.
We used a piecewise convolutional neural network that contains multiple kernel filters as our discriminative model.
This network uses multiple filters with fixed widths of 300 dimensions and a fixed height of 7 (Figure {@fig:convolutional_network}).
We choose a fixed height of 7 because this height was previously established to provide the best performance in relationship classification [@arxiv:1510.03820].
We trained this model for 15 epochs using the Adam optimizer [@arxiv:1412.6980] with pytorch's default parameter settings and a learning rate of 0.001 that decreases by half every epoch until the lower bound of 1e-5 is reached.

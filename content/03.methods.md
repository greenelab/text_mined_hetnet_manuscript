...
The challenge of having too few ground truth annotations is common too many natural language processing settings, even when unannotated text is abundant.
...
If the pair is present, our label function emits a positive label and abstain otherwise.
...
If a candidate sentence's dependency path belonged to a previously generated cluster, then the label function will emit a positive label and abstain otherwise.
...
We repeat the above process for each label function in our repertoire. 

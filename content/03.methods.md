...
Sometimes random noise can help with performance.
We discovered that adding a single label function can improve performance on top of the distant supervision model (Results Generative Model).
To better understand this anomaly we added a single label function to our distant supervision group.
This label function is designed to randomly emit either a positive or negative label at varying frequencies (five evenly spaced numbers between 0 and 1).
We trained the generative model using both distant supervision label functions and our random noise label function 
Results are reported in terms of AUROC and AUPR.

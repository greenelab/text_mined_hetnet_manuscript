...
We provide examples for the categories in our [supplemental section](### Label Function Categories).  
...
We evaluated both generative and discriminative (training and evaluation are described in the [supplemental section](### Discriminative Model))  models at each point, and we reported performance of each in terms of the area under the receiver operating characteristic curve (AUROC) and the area under the precision-recall curve (AUPR).
Lastly, we conducted a follow up experiment for the generative model described in the [supplemental section](### Adding Random Noise to Generative Model).

...
Adding a large number of edge-type-mismatch label functions produces more variable performance that depends on the edge type being predicted and the label function's edge type source.
...
We expect that future use in practice would include additional filtering and scoring methods to further enhance precision.

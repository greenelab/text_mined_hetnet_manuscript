...
The header of each line plot shows the edge type the discriminator model is trying to predict.
...
The baseline and “All” data points consisted of a fixed set of label functions, so the error bars are flat in this case.
...
The discriminator model is designed to augment performance over the generative model by incorporating textual features with generated training labels.
...
We found that the discriminative model generally out-performed the generative model as the amount of edge-specific label functions increases.
An noteable trend is that performance for the discriminator model starts off poorly when less than eleven label functions are added to the baseline model (seen in Disease associates Gene (DaG), Compound binds Gene (CbG) and Gene interacts Gene (GiG)). 
This suggests that at least eleven non-database label functions in conjunction with database related label functions is needed to generate useful training labels for the discriminator model.
One exception to this trend is Compound treats Disease (CtD), where the discriminator model out performs the generative model at all levels of sampling.
As for CbG the discriminator model performs on par with the generative model when all CbG designed label functions are used.
Interestingly, the AUPR for CbG plateaus and the decreases as the number of edge-specific label functions increases (Supplemental Figure {@fig:aupr_discriminative_model_performance}).
This suggests that the discriminator model is predicting more false positives as all CbG label functions are used.
Despite the CbG trend, the discriminator model successfully learns off of the generative model as long as an adaquate amount of label functions are incorporated to distant supervision model.

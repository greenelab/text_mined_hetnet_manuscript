...
![
Our extractor shows comparable performance with an unsupervised baseline when predicting edges in Hetionet v1.
...
We aimed to assess our model's performance relative to an unsupervised baseline [@tag:cocoscore] by comparing both models' ability to predict preexisting edges within our test set.
Our model shows comparable performance to the baseline model for the compound treats disease (CtD) edge type, while our model is not far from the baseline model when predicting other edge types (Figure {@fig:cocoscore_comparison}).
Overall, this is encouraging that with minor tweaks we can compete with previously established methods.

...
![
Weakly supervised extractors have comparable performance with unsupervised extractors when predicting preexisting edges in Hetionet v1.
...
For this project we compared our model with a previosuly published baseline model.
We calibrated our discriminator model and scored every candidate sentence in our dataset.
We grouped sentences based on mention pair (edges) and took the max score of each group to represent an edge.
We compared our model's ability to predict preexisting edges in our test set to a previously published baseline model [@tag:cocoscore].
Overall, our model shows comparable performance to the baseline model (Figure {@fig:cocoscore_comparison}).
For all edge types the baseline model outperforms our model, but for compound treats disease (CtD) performance is nearly the same.
This highlights that more advance approaches for edge representation is needed to increase performance. 

...
Due to the one label function anomaly (mentioned in Results:Generative Model Using Randomly Sampled Label Functions), we evaluated the effects of adding random noise on top of the generative model, trained using only distant supervision label functions.
In general, adding random noise does not improve performance (Figures {@fig:random_label_function_auprc} and {@fig:random_label_function_auprc}).
One exception to this trend is CbG, which had improved AUPR performance (Figure {@fig:random_label_function_auprc}).

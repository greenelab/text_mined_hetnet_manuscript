...
Even high performing deep learning models are subject to poor calibration. Typically, these models are overconfident in their predictions [@arxiv:1706.04599; @arxiv:1807.00263].
We accounted for this problem by calibrating our best performing discriminative models.
Results of the calibration process are reported in Figure {@fig:discriminative_model_calibration}.
Before calibration (green line) we found that the all edge types were somewhat aligned with the ideal calibration lines when predicting low probability scores (close to 0.25).
After applying the calibration algorithm (blue line), we found that there wasn't a drastic change in predictions.
An exception to this would be Disease associates Gene (DaG) where high predicting probabilities were brought closer to the calibration line.
Overall, Compound binds Gene (CbG) and Gene interacts Gene (GiG) model calibration failed to show any significant improvement, while DaG and Compound Treats Disease (CtD) models show some improvement in the higher scoring sections (0.75-1).
